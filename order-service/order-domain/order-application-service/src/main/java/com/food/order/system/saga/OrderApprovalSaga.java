...
public class OrderApprovalSaga implements SagaStep<RestaurantApprovalResponse> {
        ...
        public void process(RestaurantApprovalResponse restaurantApprovalResponse) {
                var orderApprovalOutboxMessageResponse = approvalOutboxHelper
                                .getApprovalOutboxMessageBySagaIdAndSagaStatus(UUID
                                                .fromString(restaurantApprovalResponse.getSagaId()),
                                                SagaStatus.PROCESSING)
                                .orElseThrow(() -> new OrderDomainException(
                                                "OrderApprovalSaga: Order approval outbox message not found"));
                ...
                approvalOutboxHelper.save(getUpdatedApprovalOutboxMessage(
                                orderApprovalOutboxMessageResponse, order.getStatus(), sagaStatus));

                paymentOutboxHelper.save(getUpdatedPaymentOutboxMessage(
                                restaurantApprovalResponse.getSagaId(), order.getStatus(),
                                sagaStatus));
        ...
        public void rollback(RestaurantApprovalResponse restaurantApprovalResponse) {
                ...
                var sagaStatus = SagaStatus.COMPENSATING;

                approvalOutboxHelper.save(
                                getUpdatedApprovalOutboxMessage(orderApprovalOutboxMessageResponse,
