...
public class PaymentOutboxHelper {
    ...
    public Optional<List<OrderPaymentOutboxMessage>> getPaymentOutboxMessageByOutboxMessageStatusAndSagaStatus(
        ...
        return paymentOutboxRepository.findByTypeAndOutboxStatusAndSagaStatus(ORDER_PROCESSING_SAGA,
    ...
    public Optional<OrderPaymentOutboxMessage> getPaymentOutboxMessageBySagaIdAndSagaStatus(
        ...
        return paymentOutboxRepository.findByTypeAndSagaIdAndSagaStatus(ORDER_PROCESSING_SAGA,
                sagaId, sagaStatus);
    }
    ...
    public void save(OrderPaymentOutboxMessage orderPaymentOutboxMessage) {
        ...
        if (Objects.isNull(response)) {
            throw new OrderDomainException(
                    "Failed to save outbox message id : " + orderPaymentOutboxMessage.getId());
        }
        log.info("Outbox message id : {} saved successfully", response.getId());
    }

    @Transactional
    public void savePaymentOutboxMessage(OrderPaymentEventPayload payload, OrderStatus orderStatus,
        ...
        save(OrderPaymentOutboxMessage.builder().id(UUID.randomUUID()).sagaId(sagaId)
                .createdAt(payload.getCreatedAt()).type(ORDER_PROCESSING_SAGA)
                .payload(createPayload(payload)).outboxStatus(outboxStatus).orderStatus(orderStatus)
                .sagaStatus(sagaStatus).build());
    ...
    public void deletePaymentOutboxMessageByOutboxStatusAndSagaStatus(OutboxStatus outboxStatus,
        ...
        paymentOutboxRepository.deleteByTypeAndOutboxStatusAndSagaStatus(ORDER_PROCESSING_SAGA,
                outboxStatus, sagaStatus);
