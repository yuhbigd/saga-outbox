package com.food.order.system.payment.data.access.outbox.adapter;
...
import com.food.order.system.payment.data.access.outbox.exception.OrderOutboxNotFoundException;
import com.food.order.system.payment.data.access.outbox.mapper.OrderOutboxDataAccessMapper;
import com.food.order.system.payment.data.access.outbox.repository.OrderOutboxJpaRepository;
...
public class OrderOutboxRepositoryImpl implements OrderOutboxRepository {
        ...
        public OrderOutboxMessage save(OrderOutboxMessage orderPaymentOutboxMessage) {
                return orderOutboxDataAccessMapper.orderOutboxEntityToOrderOutboxMessage(
                                orderOutboxJpaRepository.save(orderOutboxDataAccessMapper
                                                .orderOutboxMessageToOutboxEntity(
        ...
        public Optional<List<OrderOutboxMessage>> findByTypeAndOutboxStatus(String sagaType,
                ...
                return Optional.of(orderOutboxJpaRepository
                                .findByTypeAndOutboxStatus(sagaType, outboxStatus)
                                .orElseThrow(() -> new OrderOutboxNotFoundException(
                                                "Approval outbox object "
                                                                + "cannot be found for saga type "
                                                                + sagaType))
                                .stream()
        ...
        public Optional<OrderOutboxMessage> findByTypeAndSagaIdAndPaymentStatusAndOutboxStatus(
                        String sagaType, UUID sagaId, PaymentStatus paymentStatus,
                        OutboxStatus outboxStatus) {
                return orderOutboxJpaRepository
                                .findByTypeAndSagaIdAndPaymentStatusAndOutboxStatus(sagaType,
