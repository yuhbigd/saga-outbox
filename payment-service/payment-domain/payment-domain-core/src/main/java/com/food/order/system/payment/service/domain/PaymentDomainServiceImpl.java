...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    @Override
    public PaymentEvent validateAndCancelledPayment(Payment payment, CreditEntry creditEntry,
            List<CreditHistory> creditHistory, List<String> failureMessages) {
    ...
    private void validateCreditHistory(CreditEntry creditEntry, List<CreditHistory> creditHistory,
        ...
        if (totalDebitHistory.isGreaterThan(totalCreditHistory)) {
            ...
            log.error("Customer id {} has insufficient credit",
        ...
        if (!creditEntry.getTotalCreditAmount()
            ...
            log.error("Customer id {} has total is not equal to credit history",
                    creditEntry.getCustomerId().getValue());
        }
    }

    private Money getTotalHistoryAmount(List<CreditHistory> creditHistory,
    ...
    private void updateCreditHistory(Payment payment, List<CreditHistory> creditHistory,
            TransactionType transactionType) {
        creditHistory.add(CreditHistory.builder().id(new CreditHistoryId(UUID.randomUUID()))
                .customerId(payment.getCustomerId()).amount(payment.getPrice())
    ...
    private void validateCreditEntry(Payment payment, CreditEntry creditEntry,
        ...
        if (payment.getPrice().isGreaterThan(creditEntry.getTotalCreditAmount())) {
            failureMessages.add("Customer id " + payment.getCustomerId().getValue()
                    + " , has insufficient credit amount"
                    + creditEntry.getTotalCreditAmount().getAmount() + " to pay for order id "
