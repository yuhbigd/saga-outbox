...
public class OrderOutboxScheduler implements OutboxScheduler {
        ...
        public void processOutboxMessage() {
                var outboxMessagesResponse = orderOutboxHelper
                ...
                if (outboxMessagesResponse.isPresent() && outboxMessagesResponse.get().size() > 0) {
                        ...
                        log.info("Received {} OrderOutboxMessage with ids {}, sending to message bus!",
                                        ...
                                        outboxMessages.stream()
                                                        .map(outboxMessage -> outboxMessage.getId()
                                                                        .toString())
                                                        .collect(Collectors.joining(",")));
                        outboxMessages.forEach(orderOutboxMessage -> paymentResponseMessagePublisher
                                        .publish(orderOutboxMessage,
                                                        orderOutboxHelper::updateOutboxMessage));
                        log.info("{} OrderOutboxMessage sent to message bus!",
                                        outboxMessages.size());
                }
        }
