...
public class PaymentRequestHelper {
    ...
    public void persistPayment(PaymentRequest paymentRequest) {
        ...
        var paymentEvent = paymentDomainService.validateAndInitializePayment(payment, creditEntry,
                creditHistory, failureMessage);
        ...
        orderOutboxHelper.saveOrderOutboxMessage(
                ...
                paymentEvent.getPayment().getStatus(), OutboxStatus.STARTED, // payment status:
                                                                             // completed
    ...
    public void persistCancelPayment(PaymentRequest paymentRequest) {
        ...
        var payment = paymentRepository.findByOrderId(UUID.fromString(paymentRequest.getOrderId()))
                .orElseThrow(() -> new PaymentNotFoundException("Payment not found"));
        var creditEntry = getCreditEntry(payment.getCustomerId());
        var creditHistory = getCreditHistory(payment.getCustomerId());
        List<String> failureMessage = new ArrayList<>();
        var paymentEvent = paymentDomainService.validateAndCancelledPayment(payment, creditEntry,
                creditHistory, failureMessage);
        ...
        orderOutboxHelper.saveOrderOutboxMessage(
                paymentDataMapper.paymentEventToOrderEventPayload(paymentEvent),
                paymentEvent.getPayment().getStatus(), OutboxStatus.STARTED,
                UUID.fromString(paymentRequest.getSagaId()));

    }

    private void persistDbObject(Payment payment, CreditEntry creditEntry,
            List<CreditHistory> creditHistory, List<String> failureMessage) {
